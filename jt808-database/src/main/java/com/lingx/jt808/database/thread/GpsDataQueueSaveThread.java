...
public class GpsDataQueueSaveThread implements Runnable,ILingxThread {
		...
		while(isRun){
			try {
				if(!JT808DataService.LocatoinData.isEmpty()){
					...
					jdbcTemplate.batchUpdate(String.format("insert into %s(car_id,acc,location,alarm,status,lat,lng,height,speed,direction,gpstime,systime,mileage,oil,txxh,wxxh,status_str,alarm_str,x0704) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", this.jt808CommonService.get0x0200TableName()), 
						...
						public void setValues(PreparedStatement ps, int index) throws SQLException {
							...
							ps.setString(19, map.get("x0704").toString());
						}
	...
	public String getName() {
		return "TracSeek-Database -> 数据入库线程";
	}
