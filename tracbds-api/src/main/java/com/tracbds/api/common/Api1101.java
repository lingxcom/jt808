...
public class Api1101  extends AbstractAuthApi{
	...
	public Map<String, Object> api(Map<String, Object> params) {
		...
		if(Utils.isNotNull(text)){
			...
			if("0".equals(node)){
				list=this.jdbcTemplate.queryForList("select id,name text,id `value`,icon_cls,total,online from tgps_group where "+groupWhere2+" "+groupWhere1+" order by orderindex asc,name  asc");
				
			}else{
				list=this.jdbcTemplate.queryForList("select id,name text,id `value`,icon_cls,total,online from tgps_group where fid=? "+groupWhere1+" order by orderindex asc, name  asc",node);
			}
			 for(Map<String,Object> map:list){
				...
				if("all".equals(type)) {
					map.put("text",map.get("text")+"("+map.get("online").toString()+"/"+map.get("total").toString()+")");
				}else if("online".equals(type)) {
					map.put("text",map.get("text")+"("+map.get("online").toString()+")");
				}else if("offline".equals(type)) {
					map.put("text",map.get("text")+"("+(Integer.parseInt(map.get("total").toString())-Integer.parseInt(map.get("online").toString()))+")");
				}else if("follow".equals(type)) {
					int follow=this.jdbcTemplate.queryForObject("select count(*) from tgps_car where id in(select car_id from tgps_car_follow where user_id=?) and id in (select car_id from tgps_group_car where group_id=?)", Integer.class,userid,map.get("id"));
			 ...
			 {//根据分组ID取车辆
				 if("online".equals(type)) {
					 ...
					 isVideoWhere+=" and id in (select car_id from tgps_car_follow where user_id='"+userid+"') ";
				 }
	...
	private Map<String,Object> getData2(String userid){
		...
		int total=this.jdbcTemplate.queryForObject("select count(*) from tgps_car where id in(select car_id from tgps_group_car where group_id in(select group_id from tgps_group_user where user_id=?))", Integer.class,userid);

		int online=this.jdbcTemplate.queryForObject("select count(*) from tgps_car where online='1' and id in(select car_id from tgps_group_car where group_id in(select group_id from tgps_group_user where user_id=?))", Integer.class,userid);
