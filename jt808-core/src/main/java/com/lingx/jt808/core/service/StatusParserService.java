...
import com.lingx.jt808.core.service.statusparser.AbstractStatusParser;
import com.lingx.jt808.core.service.statusparser.LocatedStatusParser;
import com.lingx.jt808.core.service.statusparser.StatusParseBean;
...
public class StatusParserService {
	...
	@Autowired
	private StatusParserSupportService statusParserSupportService;
	...
	@PostConstruct
	public void init() {
		StatusParserSupportService.OIL_CACHE.invalidateAll();
	...
	private IJT808StatusParser getParser(String id) {
		...
		if(list.size()>0) {
			try {
				...
				if(parser instanceof AbstractStatusParser) {
					AbstractStatusParser obj=(AbstractStatusParser)parser;
					obj.setJdbcTemplate(jdbcTemplate);
					obj.setStatusParserSupportService(statusParserSupportService);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
