...
import com.lingx.jt808.core.service.ThreadPoolExecutorService;
...
public class RuleManager implements ApplicationListener<JT808Location0200Event>{
				...
				if(b) {
					...
					if(duration>=rule.getDuration()) {
						...
						if(map.containsKey("alarm_str")) {
							map.put("alarm_str", map.get("alarm_str").toString()+","+name);
						}else {
							map.put("alarm_str", name);
						}
						//System.out.println(JSON.toJSONString(map));
						action=ruleCache.getAction().addAndGet(1);
						RuleActionExecuteThread thread=new RuleActionExecuteThread(map,rule,new RuleEvent(rule.getName(),total,duration,action));
	...
	private void init() {
		...
		for(Map<String,Object> map:list) {
			...
			if(rule==null) {
				...
				System.out.println(bean.getName()+"规则实现类初始化失败");
				continue;
			}
	...
	private List<IJT808Action> getActions(Map<String,Object> map){
		...
		List<Long> instIds = SqlSafe.parseLongIds(action_inst_ids);
		if (instIds.isEmpty()) {
			return listAction;
		}
		String sql = "select t.action_config,a.clazz from tgps_action_inst t,tgps_action a where t.action_id=a.id and a.status='1' and t.id in ("
				+ SqlSafe.placeholders(instIds.size()) + ")";
		List<Map<String,Object>> list=this.jdbcTemplate.queryForList(sql, instIds.toArray());
		try {
