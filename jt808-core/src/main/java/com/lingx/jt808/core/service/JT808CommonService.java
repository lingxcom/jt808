...
public class JT808CommonService {
	...
	public String get0x0200TableName(LocalDateTime date) {
	...
	public Map<String,Object> getLast0x0200Data(String car_id){
		...
		if(temp==null) {
			...
			redisService.set(car_id, JSON.toJSONString(list.get(0)));
			return list.get(0);
		}
		return JSON.parseObject(temp);
	}
	public void refreshCache() {
		List<Map<String,Object>> list=this.jdbcTemplate.queryForList("select id from tgps_car ");
		for(Map<String,Object> map:list) {
			this.refreshCache(map.get("id"));
		}
	}
	...
	public void addJT808Info(Map<String, Object> map,String language,boolean isAddress) {
		if (map == null)
			return;
		...
		if(isAddress&&map.get("lat")!=null)
		 map.put("address",this.addressService.getAddress(map.get("lat").toString(),map.get("lng").toString()));
		else map.put("address", "-");
	}
	...
	public String getLeftCarStatusString(Map<String, Object> map,String language) {
		...
		if ("1".equals(map.get("online").toString())) {
			...
			if(map.get("gpstime")==null)return this.languageService.text("未启用", language);
	...
	public String getDtlc(String tid) {
		String temp = redisService.get("dtlc_" + tid);
		if (temp == null)
			return "0";
		else
	...
	public String getComment(Map<String, Object> map,String language) {
		...
		try {
			if ("1".equals(map.get("online").toString())) {
				...
				String changeTime = redisService.get(tid + "_speed_change");
				if (changeTime == null) {
					changeTime = Utils.getTime();
					redisService.set(tid + "_speed_change", Utils.getTime());
				}
...
*/
	...
	public void pushMessageByCarid(String carid, String content) {
		this.pushMessageByCarid(carid, content, "");
	}

	public void pushMessageByCarid(String carid, String content, String route_path) {
		String time = Utils.getTime(), userid;
		List<Map<String, Object>> list = this.jdbcTemplate.queryForList(
				"select id from tlingx_user where id in(select user_id from tgps_group_user where group_id in(select group_id from tgps_car where id=?))",
				carid);
		for (Map<String, Object> map : list) {
			userid = map.get("id").toString();
			this.jdbcTemplate.update(
	...
	public Set<String> getUserIdsById(String id) {
		...
		List<Map<String,Object>> list=this.jdbcTemplate.queryForList("select user_id from tgps_group_user where group_id in (select group_id from tgps_car where id=?)",id);
		for(Map<String,Object> map:list) {
	...
	public String tidsToIds(String tids) {
		if(Utils.isNull(tids))return tids;
		String array[]=tids.split(","),temp;
		StringBuilder sb=new StringBuilder();
		for(String str:array) {
			if(Utils.isNull(str))continue;
			
			temp=IJT808Cache.WHITE_LIST.get(Utils.parseTid(str));
			if(Utils.isNotNull(temp))
			sb.append(temp).append(",");
			else if(str.length()<=7)sb.append(str).append(",");
		}
	...
	public void deleteFile(String filepath) {
		...
		System.out.println("deleteFile:"+base.replace("jt808_server", "jt808_html")+filepath);
		File file=new File(base.replace("jt808_server", "jt808_html")+filepath);
