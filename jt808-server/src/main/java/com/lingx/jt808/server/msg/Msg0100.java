...
import com.lingx.jt808.core.cmd.Cmd8100;
import com.lingx.jt808.core.event.JT808Location0100Event;
...
public class Msg0100 extends AbstrctMsgHandler implements IJT808MsgHandler {
	...
	public void handle(ByteBuf data, String tid, int msgId, int msgSn, ChannelHandlerContext ctx,boolean isVersion,byte[] bytes12)throws Exception {
		...
		if(ret==0) {
			...
			Cmd8100 cmd=new Cmd8100(JT808Utils.getTidByVersion(tid, isVersion),msgSn,(byte)ret,code);
			ctx.writeAndFlush(cmd.toMessageByteBuf());
			
		}else {
			Cmd8100 cmd=new Cmd8100(JT808Utils.getTidByVersion(tid, isVersion),msgSn,(byte)ret);
			ctx.writeAndFlush(cmd.toMessageByteBuf());
