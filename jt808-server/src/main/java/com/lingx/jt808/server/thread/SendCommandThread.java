...
public class SendCommandThread  implements Runnable,ILingxThread {
	...
	public void run() {
		...
		try {
		while(isRun) {
				json=redisService.pop(configService.getJt808ServerId());//IJT808Cache.SEND_COMMAND_QUEUE.poll();
				if(Utils.isNotNull(json)) {
					...
					if(IJT808Cache.SESSIONS.getIfPresent(tid)==null) {
						...
						redisService.cache(key, JT808Utils.getResBeanAndToString(9,"终端设备不在线",""), 10);
					}else {
