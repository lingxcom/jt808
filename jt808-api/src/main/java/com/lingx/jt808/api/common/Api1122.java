		...
		try {
			for(Integer tid:sets) {
				...
				while(sdate.getTime()<=edate.getTime()) {
					...
					dylc=this.jdbcTemplate.queryForObject("select sum(dtlc) from tgps_mileage where car_id=? and create_time>=? and create_time<=?", Double.class,tid,month+"01",month+"31");
					if(dylc!=null) {
						map.put(key, dylc);
						sum+=dylc;
					}else {
