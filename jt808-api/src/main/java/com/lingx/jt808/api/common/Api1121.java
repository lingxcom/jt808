...
public class Api1121 extends AbstractAuthApi implements IApi{
	...
	public Map<String, Object> api(Map<String, Object> params) {
		...
		try {
			...
			Double dtlc=0d,sum;
			Integer ccts,zts;//总天数
			for(int tid:sets) {
				sum=0d;ccts=0;zts=0;
				Map<String,Object> map=new HashMap<>();
				map.putAll(this.jdbcTemplate.queryForMap("select tid,carno from tgps_car where id=?",tid));
				map.put("group", this.jdbcTemplate.queryForObject("select name from tgps_group where id in (select group_id from tgps_car where id=?)", String.class,tid));
				...
				while(sdate.getTime()<=edate.getTime()) {
					...
					dtlc=this.jdbcTemplate.queryForObject("select sum(dtlc) from tgps_mileage where car_id=? and create_time=?", Double.class,tid,sdf2.format(sdate));
					if(dtlc==null)dtlc=0d;
					map.put(key, dtlc);
					sum+=dtlc;
					if(dtlc>0)ccts++;
