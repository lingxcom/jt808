...
public class Api1163 extends AbstractAuthApi implements IApi {
	...
	public Map<String, Object> api(Map<String, Object> params) {
		...
		if("single".equals(type)) {
			...
			jdbcTemplate.update("insert into tgps_car(tid,carno,sim,czxm,tel,remark,version,create_time,modify_time,stime,etime,group_id)values(?,?,?,?,?,?,?,?,?,?,?,?)",tid,carno,sim,czxm,tel,remark,version,ts,ts,ts.substring(0,8),ets.substring(0,8),groupId);
			...
			if(channels.size()>0)addChannels(car_id,channels);
			}
		}else {
		...
		String sql = "insert into tgps_car(tid,carno,sim,czxm,tel,remark,version,create_time,modify_time,stime,etime,group_id)values(?,?,?,?,?,?,?,?,?,?,?,?)";
		tids = tids.trim();
		tids = tids.replace("，", ",");
		tids = tids.replace("\n", ",");
		...
		for (String tid : array) {
			...
			this.jdbcTemplate.update(sql, tid, tid, "","", "", remark, version, time, time,
					time.substring(0, 8), ets.substring(0, 8),groupId);
			int car_id=this.jdbcTemplate.queryForObject("select id from tgps_car where tid=?", Integer.class,tid);
			IJT808Cache.WHITE_LIST.put(tid, String.valueOf(car_id));
			if(channels.size()>0)addChannels(car_id,channels);
		}
	...
	private void addChannels(int carid,List<String> channels) {
		for (String temp22 : channels) {
			this.jdbcTemplate.update("insert into tgps_car_tdh(car_id,tdh,name)values(?,?,?)", carid,
